...
function DashboardPage() {
  ...
  const userBusinessExists = useSelector(
    (state: any) => state.businessOnboardingReducer.exists
  );

  // Effects
  useEffect(() => {
    setShowRegisterBusinessModal(!!isConnectBusiness);
  ...
  const handleFirstModalSubmit = async (values, { setSubmitting }) => {
    try {
      ...
      if (response.data.status === 200 || response?.statusCode === 200) {
        ...
        setShowRegisterBusinessModal(false);
  ...
  const handleBusinessInfoSubmit = async (values, { setSubmitting }) => {
    try {
  ...
  const handleWelcomeMessageSubmit = async (values, { setSubmitting }) => {
    try {
      const resultAction = await dispatch(
        addBusinessInfo({
          step: "step4",
    ...
    <>
      ...
      <PaymentStatusModal
        ...
        onClose={() => {
          ...
          ["ispaymentSuccess", "ispaymentRejected", "session_id"].forEach(param =>
        ...
        <>
          ...
          <div className="centered-container">
            <div className="dashboard-top-card p-4 bg-light d-flex flex-column align-items-center text-center">
              ...
              <button className="send-otp-button" onClick={() => setShowRegisterBusinessModal(true)}>
                Add Business
              </button>
            </div>
          </div>
          ...
          <Modal show={showRegisterBusinessModal} onHide={closeBusinessStepModel} centered className="custom-modal">
            <Formik
              initialValues={{
                businessName: "",
                whatsappNumber: "",
            ...
            >
              {({ handleSubmit, isSubmitting }) => (
                <Form onSubmit={handleSubmit}>
                  <Modal.Header closeButton><h2>Register your Business</h2></Modal.Header>
                  <Modal.Body className="modalBody">
                    <p className="text-left mb-3">Enter your Business Name and WhatsApp Business number</p>
                    <div className="mb-3">
                    ...
                    <button type="submit" className="send-otp-button mt-3" disabled={isSubmitting}>
                      {isSubmitting ? <span className="spin"></span> : "Send OTP"}
                    </button>
                  </Modal.Body>
                </Form>
              )}
          ...
          <Modal show={showOtpModal} onHide={() => setShowOtpModal(false)} centered backdrop="static">
            <Formik initialValues={{}} onSubmit={handleOtpVerification}>
              {({ handleSubmit, isSubmitting }) => (
                <Form onSubmit={handleSubmit}>
                  <Modal.Header closeButton><h2>Enter OTP</h2></Modal.Header>
                  <Modal.Body className="modalBody">
                    <p className="text-left mb-3">Enter the 4-digit OTP sent to <strong>{whatsappForOtp}</strong></p>
                    ...
                    <div className="resendText mt-3">Didn't receive the code? <span className="resendLink">Send again</span></div>
                    <button type="submit" className="send-otp-button mt-3" disabled={isSubmitting}>
          ...
          <Modal show={showBusinessDetailModal} onHide={() => setShowBusinessDetailModal(false)} centered className="custom-modal">
            <Formik
              initialValues={{
                industry: "",
                businessWebsite: "",
            ...
            >
              {({ handleSubmit, isSubmitting }) => (
                <Form onSubmit={handleSubmit}>
                  <Modal.Header closeButton><h2>Get Premium</h2></Modal.Header>
                  <Modal.Body className="modalBody">
                    <h5 className="mb-3">Business Info</h5>
                    <div className="mb-3">
                      <label className="form-label">What industry are you in?</label>
                      <Field name="industry" className="form-control" placeholder="e.g., Retail, Technology, Education" />
                    ...
                    <div className="mb-3">
                      ...
                      <Field name="businessWebsite" type="text" className="form-control" placeholder="https://example.com" />
                      <ErrorMessage name="businessWebsite" component="div" className="text-danger" />
                    </div>
                  </Modal.Body>
                  <Modal.Footer>
                    <button type="submit" className="send-otp-button" disabled={isSubmitting}>
                      {isSubmitting ? <span className="spin"></span> : "Submit"}
                    </button>
                  </Modal.Footer>
                </Form>
              )}
          ...
          <Modal show={showWelcomeModal} onHide={closeWelcomeMessageModal} centered>
            <Formik
              initialValues={{
              ...
              onSubmit={handleWelcomeMessageSubmit}
            >
              {({ handleSubmit }) => (
                <Form onSubmit={handleSubmit}>
                  <Modal.Header closeButton><h2>Welcome Message</h2></Modal.Header>
                  <Modal.Body className="modalBody">
                    <p className="text-left mb-3">Type your welcome message that will greet customers when they chat with you.</p>
                    <div className="mb-3">
                      <label className="form-label">Welcome Message</label>
                      <Field as="textarea" name="welcomeMessage" className="form-control" rows={3} placeholder="👋 Welcome to Pixalane! How can we help you today?" />
                      <ErrorMessage name="welcomeMessage" component="div" className="text-danger" />
                    </div>
                  </Modal.Body>
                  <Modal.Footer>
                    <button type="submit" className="send-otp-button">Save & Continue</button>
                  </Modal.Footer>
                </Form>
              )}
            </Formik>
